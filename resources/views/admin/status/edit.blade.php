...
@section('content')
    <h3 class="text-gray-700 text-3xl font-medium">Modification mon Status</h3>

    <div class="flex justify-center mt-10">
        <form action="{{ route('admin.status.edit', ['id' => $status->id ]) }}" method="POST" class="w-3/4">
            @csrf
            <div>
                ...
                <input type="text"
                       ...
                       id="name" name="name" value="{{ $status->name }}">
                @error('name')
            ...
            <div>
                ...
                <input type="text"
                       ...
                       id="color" name="color" value="{{ $status->color }}">
                @error('color')
                <span class="text-red-500 text-xs" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div>
                ...
                <input type="text"
                       ...
                       id="background" name="background" value="{{ $status->background }}">
                @error('background')
                <span class="text-red-500 text-xs" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
